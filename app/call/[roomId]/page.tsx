...
export default function CallPage({ params }: { params: { roomId: string } }) {
  ...
  const preToken = searchParams.get('token')  // caller gets token directly
  ...
  const initCall = async () => {
    try {
      ...
      // Load call info
      if (callId) {
        const { data: call } = await supabase.from('calls').select('*, caller:users!caller_id(full_name, username, avatar_url), callee:users!callee_id(full_name, username, avatar_url)').eq('id', callId).single()
        if (call) {
  ...
  const toggleMute = () => {
    ...
    callObjRef.current.setLocalAudio(muted)
    setMuted(!muted)
  }

  const toggleVideo = () => {
    if (!callObjRef.current) return
    callObjRef.current.setLocalVideo(videoOff)
  ...
  if (error) {
    return (
      <div style={{ minHeight: '100vh', background: '#080810', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif', gap: '16px' }}>
        ...
        <p style={{ fontSize: '16px', color: '#FF6B35' }}>{error}</p>
    ...
    <div style={{ minHeight: '100vh', background: '#080810', display: 'flex', flexDirection: 'column', fontFamily: 'Plus Jakarta Sans, sans-serif', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(180deg, rgba(8,8,16,0.9) 0%, transparent 100%)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: status === 'active' ? '#39FF14' : '#9B93C0', animation: status === 'active' ? 'none' : 'pulse 1.5s ease-in-out infinite' }} />
          <span style={{ fontSize: '11px', color: '#9B93C0' }}>BESTIE CALL</span>
        </div>
      </div>

      {/* Daily.co iframe fills the screen */}
      <div id="daily-call-container" style={{ flex: 1, position: 'relative', background: '#0a0a18' }}>
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', zIndex: 5 }}>
      ...
      {/* Bottom controls */}
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, padding: '20px', paddingBottom: 'max(20px, env(safe-area-inset-bottom))', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', background: 'linear-gradient(0deg, rgba(8,8,16,0.95) 0%, transparent 100%)' }}>

        {/* Mute */}
        <button onClick={toggleMute} style={{ width: '56px', height: '56px', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', background: muted ? 'rgba(255,107,53,0.2)' : 'rgba(255,255,255,0.12)', transition: 'all 0.15s' }}>
          {muted ? '🔇' : '🎤'}
        </button>

        {/* End call */}
        <button onClick={() => endCall(true)} style={{ width: '68px', height: '68px', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '26px', background: '#FF3B30', boxShadow: '0 4px 20px rgba(255,59,48,0.4)', transition: 'all 0.15s' }}>
          📵
        </button>

        {/* Video */}
        <button onClick={toggleVideo} style={{ width: '56px', height: '56px', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', background: videoOff ? 'rgba(255,107,53,0.2)' : 'rgba(255,255,255,0.12)', transition: 'all 0.15s' }}>
          {videoOff ? '📷' : '🎥'}
        </button>
      </div>
