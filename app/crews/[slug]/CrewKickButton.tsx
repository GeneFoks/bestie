...
export default function CrewKickButton({ crewId, captainId, memberId }: Props) {
  ...
  if (confirm) {
    return (
      <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
        <button onClick={() => setConfirm(false)} style={{ padding: '5px 8px', borderRadius: '7px', fontSize: '11px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#9B93C0', cursor: 'pointer' }}>
          Cancel
        </button>
  ...
  return (
    <button
      ...
      style={{ flexShrink: 0, padding: '5px 10px', borderRadius: '8px', fontSize: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#555', cursor: 'pointer' }}
    >
      ✕
    </button>
  )
