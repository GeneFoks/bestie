    ...
    <div style={{ minHeight: '100vh', background: '#09090F', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '32px 20px 120px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px', gap: '12px', flexWrap: 'wrap' }}>
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '28px', color: '#F0EAFF', margin: 0 }}>My sessions</h1>
          <Link href="/group-sessions/new" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '9px 16px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, background: 'linear-gradient(135deg, #D4AF37 0%, #B8960C 100%)', color: '#09090F', textDecoration: 'none' }}><Plus size={15} strokeWidth={2.5} /> New</Link>
        </div>
        <p style={{ fontSize: '13px', color: '#A99ECC', marginBottom: '20px' }}>Select one or more to delete them together.</p>
        ...
        {sessions.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: '#111120', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '18px' }}>
            ...
            <p style={{ fontSize: '14px', color: '#A99ECC', marginBottom: '14px' }}>You haven't hosted any sessions yet.</p>
            <Link href="/group-sessions/new" style={{ fontSize: '13px', fontWeight: 700, color: '#D4AF37', textDecoration: 'none' }}>Create your first →</Link>
          </div>
        ) : (
          <>
            <button onClick={toggleAll} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', color: '#A99ECC', fontSize: '13px', cursor: 'pointer', marginBottom: '12px', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {sessions.map(s => {
                ...
                return (
                  <div key={s.id} onClick={() => toggle(s.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '14px', background: on ? 'rgba(212,175,55,0.08)' : '#111120', border: on ? '1px solid rgba(212,175,55,0.4)' : '1px solid rgba(255,255,255,0.10)', cursor: 'pointer', opacity: isPast ? 0.6 : 1 }}>
                    <span style={{ flexShrink: 0, width: '22px', height: '22px', borderRadius: '6px', border: on ? 'none' : '2px solid rgba(255,255,255,0.25)', background: on ? '#D4AF37' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {on && <Check size={14} color="#09090F" strokeWidth={3} />}
                    </span>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '14px', fontWeight: 600, color: '#F0EAFF', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.title}</span>
                        ...
                        {isPast && <span style={{ fontSize: '10px', color: '#6B6490' }}>past</span>}
                      </div>
                      <p style={{ fontSize: '12px', color: '#A99ECC', margin: 0 }}>{fmt(s.scheduled_at)}{s.location ? ` · ${s.location}` : ''} · {s.participants?.[0]?.count || 0} joined</p>
                    </div>
                  </div>
                )
              })}
      ...
      {/* Sticky delete bar */}
      {selected.size > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 60, padding: '14px 20px calc(16px + env(safe-area-inset-bottom))', background: 'rgba(8,8,16,0.97)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(255,255,255,0.10)' }}>
          <div style={{ maxWidth: '680px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
            <span style={{ fontSize: '14px', color: '#F0EAFF', fontWeight: 600 }}>{selected.size} selected</span>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setSelected(new Set())} style={{ padding: '11px 16px', borderRadius: '11px', fontSize: '13px', fontWeight: 600, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#A99ECC', cursor: 'pointer' }}>Clear</button>
