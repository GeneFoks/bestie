  ...
  if (denied) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '16px', color: 'var(--text-primary)' }}>Admins only</p>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', background: 'var(--nav-bg)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
          ...
          <Link href="/admin/moderation" style={{ fontSize: '13px', color: 'var(--text-muted)', textDecoration: 'none' }}>Moderation</Link>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '36px 20px 100px' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '24px' }}>{pending.length} pending · {apps.length} total</p>
        ...
        {apps.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: '18px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted)' }}>No applications yet.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {apps.map(a => (
              <div key={a.id} style={{ background: 'var(--surface-1)', border: `1px solid ${a.status === 'pending' ? 'rgba(212,175,55,0.3)' : 'var(--border)'}`, borderRadius: '16px', padding: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: a.message ? '10px' : '12px' }}>
                  <Link href={`/${a.applicant?.username}`} target="_blank" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none', flex: 1, minWidth: 0 }}>
                    <span style={{ width: '40px', height: '40px', borderRadius: '11px', overflow: 'hidden', background: 'var(--surface-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ minWidth: 0 }}>
                      <span style={{ display: 'block', fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>{a.applicant?.full_name}</span>
                      <span style={{ display: 'block', fontSize: '12px', color: 'var(--text-muted)' }}>
                        @{a.applicant?.username}{a.applicant?.city ? ` · ${a.applicant.city}` : ''} · ★ {a.applicant?.bestie_score}
                      </span>
                    </span>
                  </Link>
                  <span style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px', color: a.status === 'pending' ? '#FF6B35' : a.status === 'approved' ? '#34D399' : 'var(--text-dim)' }}>
                    {a.status}
                  </span>
                </div>
                {a.message && <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px', lineHeight: 1.5 }}>«{a.message}»</p>}
