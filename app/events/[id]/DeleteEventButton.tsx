...
export default function DeleteEventButton({ eventId, captainId, crewSlug, eventTitle }: Props) {
  ...
  if (step === 'deleting') {
    return (
      <div style={{ marginTop: '12px', padding: '14px', borderRadius: '14px', textAlign: 'center', fontSize: '13px', color: 'var(--text-muted)', background: 'rgba(255,107,53,0.06)', border: '1px solid rgba(255,107,53,0.15)' }}>
        Deleting…
      </div>
    )
  }
  ...
  if (step === 'confirm') {
    return (
      <div style={{ marginTop: '12px', padding: '16px', borderRadius: '14px', background: 'rgba(255,107,53,0.08)', border: '1px solid rgba(255,107,53,0.25)' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '14px' }}>All attendees will be removed. This cannot be undone.</p>
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={() => setStep('idle')} style={{ flex: 1, padding: '10px', borderRadius: '10px', fontSize: '13px', fontWeight: 600, background: 'var(--overlay-2)', color: 'var(--text-muted)', border: '1px solid var(--border)', cursor: 'pointer' }}>
            Cancel
          </button>
        </div>
      </div>
