    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '28px', color: 'var(--text-primary)', marginBottom: '8px' }}>Event not found</h1>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        {/* Event Card */}
        <div style={{ background: 'linear-gradient(135deg, var(--surface-1) 0%, var(--surface-3) 100%)', border: '1px solid rgba(212,175,55,0.25)', borderRadius: '28px', padding: '32px', marginBottom: '20px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', flexWrap: 'wrap', gap: '8px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
              ...
              {coHosts && coHosts.length > 0 && coHosts.map((ch: any) => ch.crew && (
                <span key={ch.crew.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: 'var(--text-muted)' }}>
          ...
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '32px', color: 'var(--text-primary)', marginBottom: '20px', lineHeight: 1.2 }}>{event.title}</h1>

          {/* Date / Location */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ fontSize: '18px' }}>📅</span>
              <span style={{ fontSize: '15px', color: 'var(--text-primary)', fontWeight: 600 }}>{dateStr} · {timeStr}</span>
            </div>
            {event.location && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '18px' }}>📍</span>
                <span style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{event.location}</span>
              </div>
            )}
          </div>

          {event.description && (
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: '20px' }}>{event.description}</p>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: '8px', marginBottom: '24px' }}>
            ...
            {spotsLeft !== null && (
              <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: '14px', padding: '12px 14px', border: '1px solid var(--border)' }}>
                <p style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '1px', color: 'var(--text-muted)', marginBottom: '4px' }}>SPOTS LEFT</p>
                <div style={{ fontSize: '26px', fontWeight: 700, color: spotsLeft <= 3 ? '#FF6B35' : 'var(--text-primary)', fontFamily: 'DM Serif Display, serif' }}>{spotsLeft}</div>
              </div>
            )}
          </div>
        ...
        {goingList.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {goingList.map(({ user }) => {
              ...
              return (
                <Link key={user.id} href={`/${user.username}`} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: 'var(--surface-1)', border: '1px solid var(--border)', borderRadius: '14px', textDecoration: 'none' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '12px', overflow: 'hidden', background: 'var(--surface-3)', flexShrink: 0, border: '1px solid rgba(212,175,55,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {user.avatar_url
              ...
              {maybeList.map(({ user }) => {
                ...
                return (
                  <Link key={user.id} href={`/${user.username}`} title={user.full_name} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '6px 10px', borderRadius: '999px', background: 'rgba(212,175,55,0.08)', border: '1px solid rgba(212,175,55,0.20)', textDecoration: 'none' }}>
                    <div style={{ width: '22px', height: '22px', borderRadius: '50%', overflow: 'hidden', background: 'var(--surface-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {user.avatar_url
                        ? <img src={user.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : <span style={{ fontSize: '10px', fontWeight: 700, color: '#D4AF37' }}>{user.full_name?.[0]}</span>
                      }
                    </div>
                    <span style={{ fontSize: '12px', color: 'var(--text-primary)', fontWeight: 600 }}>{user.full_name?.split(' ')[0]}</span>
                  </Link>
                )
              })}
            </div>
