    ...
    <div style={{ minHeight: '100vh', background: '#080810', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start', marginBottom: '32px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', flexWrap: 'wrap' }}>
              ...
              {profile.is_verified && <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '999px', background: 'rgba(212,175,55,0.15)', border: '1px solid rgba(212,175,55,0.3)', color: '#D4AF37', fontWeight: 600 }}>✓ Verified</span>}
            </div>
            <p style={{ fontSize: '14px', color: '#9B93C0', marginBottom: '12px' }}>📍 {profile.city}{profile.country ? `, ${profile.country}` : ''} · @{profile.username}</p>
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '10px', background: `rgba(${scoreColor === '#39FF14' ? '57,255,20' : '212,175,55'},0.1)`, border: `1px solid ${scoreColor}30` }}>
                <span style={{ fontSize: '12px', color: '#9B93C0' }}>BS</span>
                <span style={{ fontSize: '16px', fontWeight: 700, color: scoreColor }}>{score}</span>
              </div>
              <div style={{ fontSize: '14px', color: '#9B93C0', display: 'flex', alignItems: 'center', gap: '4px' }}>⭐ {profile.avg_rating} · {profile.total_sessions} sessions</div>
            </div>
          </div>
        </div>
          ...
          <div style={{ background: '#0F0F1E', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', padding: '20px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {profile.lights.map((light) => (
                <span key={light} style={{ padding: '6px 14px', borderRadius: '999px', fontSize: '13px', fontWeight: 500, background: 'rgba(57,255,20,0.08)', border: '1px solid rgba(57,255,20,0.2)', color: '#39FF14' }}>✦ {light}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          {profile.activity_packages.map((pkg, i) => (
            <div key={i} style={{ background: '#0F0F1E', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', padding: '20px', marginBottom: '12px' }}>
              ...
              <Link href="/signup" style={{ display: 'block', marginTop: '16px', padding: '12px', borderRadius: '12px', textAlign: 'center', fontSize: '14px', fontWeight: 600, background: 'linear-gradient(135deg, #D4AF37 0%, #B8960C 100%)', color: '#080810', textDecoration: 'none' }}>Book a Session →</Link>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.08) 0%, rgba(57,255,20,0.04) 100%)', border: '1px solid rgba(212,175,55,0.15)', borderRadius: '16px', padding: '24px', textAlign: 'center' }}>
          ...
          <Link href="/signup" style={{ display: 'inline-block', padding: '10px 24px', borderRadius: '12px', fontSize: '14px', fontWeight: 600, background: 'linear-gradient(135deg, #D4AF37 0%, #B8960C 100%)', color: '#080810', textDecoration: 'none' }}>Build your Social Passport →</Link>
        </div>
      </div>
    </div>
  )
