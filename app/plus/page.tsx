    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ maxWidth: '520px', margin: '0 auto', padding: '48px 24px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          ...
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '34px', color: 'var(--text-primary)', margin: '0 0 10px' }}>
            Unlock everything for {PLUS_PRICE}<span style={{ fontSize: '18px', color: 'var(--text-muted)' }}>/mo</span>
          </h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '32px' }}>
          {PLUS_FEATURES.map((f) => (
            <div key={f.title} style={{ display: 'flex', gap: '14px', padding: '18px', borderRadius: '16px', background: 'var(--overlay)', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '26px', lineHeight: 1 }}>{f.icon}</div>
              <div>
                <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: '15px', margin: '0 0 4px' }}>{f.title}</p>
                <p style={{ color: 'var(--text-muted)', fontSize: '13px', lineHeight: 1.5, margin: 0 }}>{f.body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '14px' }}>Loading…</div>
        ) : active ? (
