...
export default function CryptoTip({ address, chain, name }: { address: string; chain?: string; name?: string }) {
  ...
  return (
    <div style={{ background: '#111120', border: '1px solid rgba(212,175,55,0.22)', borderRadius: '18px', padding: '18px', marginBottom: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
        ...
        <div>
          <p style={{ fontSize: '14px', fontWeight: 700, color: '#F0EAFF', margin: 0 }}>Tip {name || 'them'} in crypto</p>
          <p style={{ fontSize: '11px', color: '#6B6490', margin: 0 }}>{chain || 'Crypto'} · straight to their wallet, no middleman</p>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '12px' }}>
        <code style={{ flex: 1, minWidth: 0, fontSize: '13px', color: '#D4AF37', background: '#0F0F1E', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '10px 12px', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {short}
        </code>
